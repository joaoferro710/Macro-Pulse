...
from ingestion.loader import get_active_backend, get_connection, get_series, load_all
...
TOPIC_OPTIONS = {
    ...
    "en": {
        ...
        "fx": "FX and commodities",
    },
}
COPY = {
    "pt": {
        "caption": "Painel macroeconomico com indicadores reais, anomalias quantitativas e briefings automatizados.",
        "sidebar": "Status das fontes, regimes atuais e controle de atualizacao.",
        "refresh": "Atualizar dados",
        "refreshing": "Atualizando FRED, BCB e Alpha Vantage...",
        "indicators": "Painel de indicadores",
        "charts": "Graficos de series temporais",
        "pick_series": "Escolha a serie para visualizar os ultimos 36 periodos",
        "briefing": "Briefing do agente",
        "topic": "Topico do briefing",
        "generate": "Gerar novo briefing",
        "generating": "Gerando briefing com o agente macroeconomico...",
        "generated": "Briefing gerado com sucesso.",
        "briefing_error": "Nao foi possivel gerar o briefing",
        "raw": "Conteudo bruto do briefing",
        "compare": "Comparativo BR x EUA",
        "compare_caption": "Comparacao ilustrativa com USD 10.000 investidos em cada ativo de mercado do escopo.",
        "compare_intro": "O objetivo aqui e comparar o comportamento do mesmo capital em EWZ e SPY ao longo dos ultimos 12 meses, identificar qual mercado remunerou melhor e contextualizar uma leitura prudente para os proximos 3 meses.",
        "evolution": "Evolucao simulada do investimento em 12 meses",
        "readout": "Leitura comparativa",
        "forecast": "Cenario ilustrativo para os proximos 3 meses",
        "disclaimer": "Este bloco e apenas uma leitura quantitativa do cenario atual. Nao constitui recomendacao de investimento.",
        "winner": "Melhor rendimento em 12 meses",
        "normal": "Normal",
        "anomaly": "Anomalia detectada",
        "variation": "Variacao vs. periodo anterior",
        "last_date": "Ultima data",
        "no_briefing": "Nenhum briefing foi gerado ainda.",
        "translate_error": "Nao foi possivel traduzir o briefing automaticamente. Exibindo o texto original.",
        "no_data": "Sem dados para a serie selecionada.",
        "no_anomalies": "Nenhuma anomalia por Z-score foi detectada nos ultimos 36 periodos.",
        "anomaly_caption": "Pontos vermelhos destacam anomalias detectadas por Z-score.",
        "source_status": "Status das fontes",
        "regimes": "Regimes atuais",
        "rows": "Linhas",
        ...
        "end": "Preco final",
        "ret": "Retorno 12m",
        "final": "Valor final (USD)",
        "date": "Data",
        "value": "Valor",
        "constructive": "cenario construtivo",
        "neutral": "cenario neutro",
        "cautious": "cenario cauteloso",
        "briefing_topic": "Topico",
        "backend_local": "MotherDuck indisponivel no momento. O dashboard esta usando o banco local para leitura.",
        "backend_local_detail": "Origem atual: banco local",
    },
    "en": {
        ...
        "refreshing": "Refreshing FRED, BCB and Alpha Vantage...",
        ...
        "generating": "Generating the macro briefing...",
        "generated": "Briefing generated successfully.",
        "briefing_error": "Unable to generate the briefing",
        "raw": "Raw briefing content",
        "compare": "Brazil vs US comparison",
        "compare_caption": "Illustrative comparison assuming USD 10,000 invested in each in-scope market asset.",
        ...
        "evolution": "Illustrative 12-month investment path",
        "readout": "Comparative readout",
        "forecast": "Illustrative 3-month outlook",
        "disclaimer": "This section is only a quantitative interpretation of the current setup. It is not investment advice.",
        "winner": "Best 12-month performer",
        "normal": "Normal",
        "anomaly": "Anomaly detected",
        "variation": "Change vs. previous period",
        "last_date": "Latest date",
        "no_briefing": "No briefing has been generated yet.",
        "translate_error": "Unable to translate the briefing automatically. Showing the original text instead.",
        "no_data": "No data available for the selected series.",
        "no_anomalies": "No Z-score anomalies were detected in the last 36 periods.",
        "anomaly_caption": "Red points highlight Z-score anomalies.",
        ...
        "end": "End price",
        "ret": "12M return",
        "final": "Final value (USD)",
        "date": "Date",
        "value": "Value",
        "constructive": "constructive setup",
        "neutral": "neutral setup",
        "cautious": "cautious setup",
        "briefing_topic": "Topic",
        "backend_local": "MotherDuck is currently unavailable. The dashboard is using the local database for reads.",
        "backend_local_detail": "Current source: local database",
    },
}
...
def translate_briefing(content: str, target_lang: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Translate this Brazilian Portuguese macroeconomic briefing into English. Preserve numbers, dates and indicator names. Return only the translation.",
            },
...
def indicator_catalog() -> pd.DataFrame:
    with get_connection(read_only=True) as con:
        df = con.execute(
            "SELECT DISTINCT series_id, series_name, source FROM economic_indicators"
        ).fetchdf()
    df["sort_key"] = df["series_id"].apply(
        lambda x: SERIES_ORDER.index(x) if x in SERIES_ORDER else 999
    )
...
def source_status() -> pd.DataFrame:
    with get_connection(read_only=True) as con:
        return con.execute(
            "SELECT source, MAX(loaded_at) AS last_loaded_at, COUNT(*) AS total_rows FROM economic_indicators GROUP BY source ORDER BY source"
        ).fetchdf()
...
def cards() -> list[dict[str, Any]]:
    ...
    for row in indicator_catalog().itertuples(index=False):
        ...
        latest_anomaly = bool(
            analysis["zscore_result"].iloc[-1]["is_anomaly"]
        ) if not analysis["zscore_result"].empty else False
        out.append(
            {
...
def investment_comparison() -> dict[str, Any]:
    ...
    for series_id, region in [("EWZ", "BR"), ("SPY", "US")]:
            ...
            evolution.append(
                {
                    ...
                    "series_id": series_id,
                    "region": region,
                    "portfolio_value": units * float(row.value),
                }
            )
        recent = get_series(series_id, n_periods=4)
        momentum = (
            float(recent.iloc[-1]["value"]) / float(recent.iloc[0]["value"]) - 1
        ) if len(recent) >= 2 else 0.0
        latest_anomaly = bool(
            analyze_series(
                series_id, n_periods=min(36, len(get_series(series_id, 36)))
            )["zscore_result"].iloc[-1]["is_anomaly"]
        )
        regime = (
            snapshot["united_states"]["current_regime"]
            if region == "US"
            else snapshot["brazil"]["current_regime"]
        )
        bias = (
            {"normal": 0.015, "flat": 0.0, "inverted": -0.02}.get(regime, 0.0)
            if region == "US"
            else {
                "expansao": 0.02,
                "estabilidade": 0.005,
        ...
        expected = max(
            min((momentum * 0.6) + bias + (-0.01 if latest_anomaly else 0.0), 0.12),
            -0.12,
        )
        rows.append(
            {
                ...
                "expected_return_3m": expected,
                "projected_value_3m": INITIAL_INVESTMENT_USD * (1 + expected),
                "regime": regime,
                "latest_anomaly": latest_anomaly,
                "outlook": (
                    "constructive"
                    if expected >= 0.04
                    else "cautious" if expected <= -0.02 else "neutral"
                ),
                "end_date": hist.iloc[-1]["date"].date().isoformat(),
            }
        )
    table = pd.DataFrame(rows).sort_values("return_pct", ascending=False).reset_index(
        drop=True
    )
    evo = pd.DataFrame(evolution).sort_values(["date", "series_id"]).reset_index(
        drop=True
    )
...
def _language_toggle() -> None:
    ...
    if col1.button("PT", use_container_width=True, type="primary" if current == "pt" else "secondary"):
        if current != "pt":
            st.session_state["lang"] = "pt"
            st.rerun()
    if col2.button("EN", use_container_width=True, type="primary" if current == "en" else "secondary"):
        if current != "en":
...
def _sidebar(snapshot: dict[str, Any]) -> None:
    st.sidebar.markdown(
        "<div style='font-size:1.3rem;font-weight:800;letter-spacing:.04em;color:#f0c040;padding-bottom:.5rem;'>Macro Pulse</div>",
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"<span class='pill regime-{snapshot['united_states']['current_regime']}'>{tr('us')}: {snapshot['united_states']['current_regime']}</span>",
        unsafe_allow_html=True,
    )
    st.sidebar.markdown(
        f"<span class='pill regime-{snapshot['brazil']['current_regime']}'>{tr('brazil')}: {snapshot['brazil']['current_regime']}</span>",
        unsafe_allow_html=True,
    )
    st.sidebar.divider()
    ...
    for row in source_status().itertuples(index=False):
        ...
        st.sidebar.markdown(
            f"<div class='source-box'><b>{row.source}</b><br><span style='font-size:.8rem;color:#94a3b8'>{tr('updated')}: {last_loaded:%Y-%m-%d %H:%M}</span><br><span style='font-size:.8rem;color:#94a3b8'>{tr('rows')}: {row.total_rows:,}</span></div>",
            unsafe_allow_html=True,
        )
    ...
    for idx, card in enumerate(cards()):
        with cols[idx % 3]:
            with st.container():
                ...
                st.markdown(
                    f"<div class='card'><div><div class='label'>{card['source']} | {card['series_id']}</div><div class='value'>{card['value']:,.2f}</div><div class='meta'>{card['series_name']}</div><div class='meta'>{tr('variation')}: {arrow} {card['delta']:,.2f}</div><div class='meta'>{tr('last_date')}: {card['date']}</div></div><div><span class='badge {badge}'>{label}</span></div></div>",
                    unsafe_allow_html=True,
                )
...
def _render_chart() -> None:
    ...
    options = {
        f"[{row.source}] {row.series_name} ({row.series_id})": row.series_id
        for row in indicator_catalog().itertuples(index=False)
    }
    ...
    if series_df.empty:
        ...
        base = alt.Chart(series_df).properties(
            height=360,
            title=alt.TitleParams(text=selected, fontSize=14, anchor="start"),
        )
        line = base.mark_line(color="#0f4c81", strokeWidth=3, clip=True).encode(
            x=alt.X("date:T", title=tr("date")),
            y=alt.Y("value:Q", title=tr("value")),
            tooltip=[
                alt.Tooltip("date:T", title=tr("date")),
                alt.Tooltip("value:Q", title=tr("value"), format=",.2f"),
            ],
        )
        points = alt.Chart(anomaly_df).mark_circle(size=120, color="#d62828", clip=True).encode(
            x="date:T",
            y="value:Q",
            tooltip=[
                alt.Tooltip("date:T", title=tr("date")),
...
def _render_briefing() -> None:
    ...
    if latest is None:
        ...
        st.caption(
            f"{tr('briefing_topic')}: {latest['topic']} | {generated_at:%Y-%m-%d %H:%M:%S}"
        )
...
def _render_comparison() -> None:
    ...
    top = st.columns(3)
    top[0].metric("EWZ BR", f"USD {ewz['final_value']:,.2f}", delta=f"{ewz['return_pct']:+.2%}", delta_color="normal")
    top[1].metric("SPY US", f"USD {spy['final_value']:,.2f}", delta=f"{spy['return_pct']:+.2%}", delta_color="normal")
    ...
    chart = (
        ...
        .encode(
            x=alt.X("date:T", title=tr("date")),
            y=alt.Y("portfolio_value:Q", title=tr("final")),
            color=alt.Color("label:N", title=tr("asset")),
            tooltip=[
                alt.Tooltip("date:T", title=tr("date")),
                alt.Tooltip("label:N", title=tr("asset")),
                alt.Tooltip("portfolio_value:Q", title=tr("final"), format=",.2f"),
            ],
        )
    ...
    st.dataframe(
        shown[
            ["series_id", "region", "start_price", "end_price", "return_pct", "final_value"]
        ].rename(
            columns={
                "series_id": tr("asset"),
                "region": tr("market"),
                "start_price": tr("start"),
                "end_price": tr("end"),
                "return_pct": tr("ret"),
                "final_value": tr("final"),
            }
        ),
    ...
    summary = (
        f"Nos ultimos 12 meses, USD {INITIAL_INVESTMENT_USD:,.0f} aplicados em EWZ teriam virado USD {ewz['final_value']:,.2f}, enquanto o mesmo valor em SPY teria chegado a USD {spy['final_value']:,.2f}. O recorte favoreceu o mercado {'brasileiro' if ewz['final_value'] > spy['final_value'] else 'americano'}, mas a leitura macro muda o contexto: os EUA seguem em regime {data['snapshot']['united_states']['current_regime']}, enquanto o Brasil esta em {data['snapshot']['brazil']['current_regime']}."
    ...
    st.markdown(
        f"<div class='note'><strong>{tr('readout')}</strong><br><br>{summary}</div>",
        unsafe_allow_html=True,
    )
    ...
    for col, row in zip(cols, [ewz, spy]):
        ...
        text = (
            f"**{row['series_id']} - {_region(row['region'])}**  \nLeitura atual: {outlook}.  \nMovimento recente de 3 meses: {row['momentum_3m']:.2%}.  \nProjecao ilustrativa de 3 meses: {row['expected_return_3m']:.2%}.  \nValor ilustrativo ao fim do periodo: USD {row['projected_value_3m']:,.2f}.  \nBase da leitura: regime {row['regime']}, ultimo fechamento em {row['end_date']}."
            if st.session_state["lang"] == "pt"
...
def main() -> None:
    ...
    if backend["kind"] == "local":
        detail = (
            f"{tr('backend_local_detail')}: `{backend['detail']}`"
            if backend["detail"]
            else tr("backend_local_detail")
        )
        st.warning(f"{tr('backend_local')} {detail}")
    st.markdown(
        """
